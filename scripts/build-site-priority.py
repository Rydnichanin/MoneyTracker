...
def patch_priority_loading(body: str) -> str:
    ...
    const loadDateRange = async (from, to, useCache = true) => {
      ...
      if (useCache && currentRangeCache.has(key)) {
        allTx = currentRangeCache.get(key).slice();
        window.allTx = allTx;
        debouncedRender();
        return;
      }
    ...
    new = '''      } else { f.value=""; t.value=""; dateRow.style.display='none'; }
      ...
      if (mode === 'yesterday') {
        const y = f.value;
        loadDateRange(y, y).then(() => render()).catch(e => {
          console.warn('[Priority] Yesterday load failed:', e.message);
          render();
        });
        return;
      }
      ...
      if (mode === 'custom') {
        const from = f.value;
        const to = t.value || from;
        if (!from) { render(); return; }
        loadDateRange(from, to).then(() => render()).catch(e => {
          console.warn('[Priority] Custom range load failed:', e.message);
          render();
        });
        return;
      }
