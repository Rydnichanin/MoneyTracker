/* AI WhatsApp parser v3 — Firebase-first semantic parser */
(function(){
  const VERSION='3.0-firebase-first';
  let questions=[];
  ...
  window.parseWA=async()=>{
    ...
    try{
      ...
      const ref=db.slice(0,1000).map(a=>`${a.addr}|${a.type}|${a.price||''}`).join('\n');
      const prompt=`Ты умный разборщик курьерских сообщений, версия ${VERSION}. ПРАВИЛО №1: перед любым решением по адресу сначала используй справочник Firebase, переданный ниже. Если адрес найден или однозначно совпадает с записью — ОБЯЗАТЕЛЬНО бери type и price из базы. Не заменяй тариф базы своим предположением. ПРАВИЛО №2: если адреса нет, анализируй смысл сообщения и формат адреса самостоятельно. КВ, П, ПОД не обязательны. Телефоны никогда не считать адресом или квартирой. Примеры обучения: "1-24-4п" = до подъезда; "4-1-105 (домофон 175)" = квартира; "1-20-36" = квартира. Гаражи и коттеджи = как до подъезда; Колизей = как до подъезда 150; Валерьяновка/Октябрьский = 2500; дачи до домика = 2800; дачи до КПП = 2000; речка = 1500; домик газовика = 1000. Обычные F1/F2/F3: подъезд 150, квартира 300. ALLEY PUB: подъезд 600, квартира 900. ПРАВИЛО №3: если уверенность ниже 0.75 или есть два реально возможных варианта — НЕ УГАДЫВАЙ, добавь вопрос. ПРАВИЛО №4: после ответа пользователя запись можно сохранить как выученный адрес. Верни только JSON: {"orders":[{"point":"F1","address":"...","type":"under|flat|other","amount":150,"msgTime":"20:04","confidence":0.9}],"questions":[{"point":"F1","address":"...","msgTime":"20:04","reason":"...","confidence":0.5}]}. Справочник Firebase:\n${ref||'(пустой)'}\nСообщения:\n${text}`;
      let out;
      if(gem){const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${key}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({system_instruction:{parts:[{text:prompt}]},contents:[{role:'user',parts:[{text:'Разбери сообщения.'}]}],generationConfig:{response_mime_type:'application/json',temperature:.1}})});const d=await r.json();if(d.error)throw Error(d.error.message);out=JSON.parse(d.candidates?.[0]?.content?.parts?.[0]?.text||'{}')}
      else{const r=await fetch(base+'/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${key}`},body:JSON.stringify({model,temperature:.1,response_format:{type:'json_object'},messages:[{role:'system',content:prompt},{role:'user',content:'Разбери сообщения.'}]})});const raw=await r.text();const d=JSON.parse(raw);if(d.error)throw Error(d.error.message);let t=d.choices?.[0]?.message?.content||'{}';t=t.replace(/^```json\s*/i,'').replace(/^```\s*/i,'').replace(/\s*```$/,'');out=JSON.parse(t)}
