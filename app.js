...
function render() {
    ...
    // --- ВОЗМОЖНЫЙ ДОХОД (Без КАРГО, расшифровка 600,900,1000,2000) ---
    let totalGain = 0;
    const vdStats = {};
    const pts = ["F1", "F2", "F3", "Ночь"]; // Убрали Карго
    ...
    const vdHtml = Object.entries(vdStats).map(([p, data]) => {
        ...
        return `
            <div class="stat-row">
                ...
                <div class="stat-sub" style="color:#555">${breakdownStr}</div>
    ...
    if (totalGain > 0) {
        document.getElementById("potentialStats").innerHTML += `<div class="gain-box"><span>ОБЩАЯ ВЫГОДА:</span><span class="pos">+${totalGain.toLocaleString()} ₸</span></div>`;
    }
