...
public class MainActivity extends Activity {
    ...
    protected void onResume() {
        super.onResume();
        if (pageLoaded) {
            // Страница уже загружена — проверяем токен с задержкой 500мс
            handler.postDelayed(this::checkAuthFromChrome, 500);
        }
    }
    ...
    private void checkAuthFromChrome() {
        webView.evaluateJavascript(
            "(function() {" +
            "  var token = localStorage.getItem('mt_auth_idtoken');" +
            "  var name = localStorage.getItem('mt_auth_name');" +
            "  var email = localStorage.getItem('mt_auth_email');" +
            "  if (token) {" +
            "    localStorage.removeItem('mt_auth_idtoken');" +
            "    return JSON.stringify({idToken: token, name: name, email: email});" +
            "  }" +
            "  return null;" +
            "})()",
            value -> {
