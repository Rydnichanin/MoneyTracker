...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Проверяем deep link при первом запуске
    ...
    protected void onNewIntent(Intent intent) {
        ...
        // Получаем deep link moneytracker://auth?idToken=...
    ...
    private void handleDeepLink(Intent intent) {
        ...
        if ("moneytracker".equals(data.getScheme()) && "auth".equals(data.getHost())) {
            ...
            if (idToken != null) {
                // Ждём загрузки страницы потом передаём токен
                passTokenToWebView(idToken, name, email);
            }
        }
    }

    private void passTokenToWebView(String idToken, String name, String email) {
        Runnable runnable = () -> {
            String js = "window.receiveAuthToken && window.receiveAuthToken({" +
